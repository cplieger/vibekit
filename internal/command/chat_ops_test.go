...
func closeCmd(t *testing.T) *vibekit.ClientCommand {
	t.Helper()
...
func TestCmdCloseChat_TearsDownAChatWithNoBridge(t *testing.T) {
	deps := &closeDeps{benchDeps: newBenchDeps()}

	if _, err := CmdCloseChat(t.Context(), deps, deps, deps, closeCmd(t)); err != nil {
		t.Fatalf("CmdCloseChat with no bridge = %v, want it to succeed", err)
	}
...
func TestCmdCloseChat_CancelsTheTurnAndLogsNoFailure(t *testing.T) {
	logs := captureLogs(t)
	bridge := &cancelBridge{}
	deps := &closeDeps{benchDeps: newBenchDeps(), bridge: bridge}

	if _, err := CmdCloseChat(t.Context(), deps, deps, deps, closeCmd(t)); err != nil {
		t.Fatalf("CmdCloseChat = %v, want it to succeed", err)
	}
