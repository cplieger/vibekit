...
func TestPolicyExplainMapsCamelCase(t *testing.T) {
	...
	res, err := h.config.PolicyExplain(t.Context(), vibekit.PolicyExplainRequest{Capability: "fs_write", Resource: "/x/y"})
	if err != nil {
...
func TestPolicyExplainRequiresTarget(t *testing.T) {
	h, _, br := newTestHub()
	seedPolicy(br, `{}`, `{}`)
	if _, err := h.config.PolicyExplain(t.Context(), vibekit.PolicyExplainRequest{}); err == nil {
