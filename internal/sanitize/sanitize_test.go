...
func TestIsHiddenUnicode_classifies_every_branch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isHidden(tt.r); got != tt.want {
				t.Errorf("isHidden(U+%04X) = %v, want %v", tt.r, got, tt.want)
			}
		})
	}
}
...
func TestSanitizeUnicode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := Unicode(tt.in)
			if got != tt.want {
				t.Errorf("Unicode(%q) = %q, want %q", tt.in, got, tt.want)
			}
		})
	}
}
...
func TestSanitizeUnicode_is_idempotent(t *testing.T) {
	...
	for _, in := range inputs {
		...
		twice := Unicode(once)
		if once != twice {
			t.Errorf("Unicode not idempotent: %q → %q → %q", in, once, twice)
		}
	}
}
...
func TestSanitizeOutput_is_idempotent(t *testing.T) {
	...
	for _, in := range inputs {
		...
		twice := Output(once)
		if once != twice {
			t.Errorf("Output not idempotent: %q → %q → %q", in, once, twice)
		}
	}
}
...
func TestSanitizeOutput_strips_ansi_before_unicode(t *testing.T) {
	...
	for _, in := range tricky {
		got := Output(in)
		want := Unicode(StripANSI(in))
		if got != want {
			t.Errorf("Output(%q) = %q, want Unicode(StripANSI(_)) = %q", in, got, want)
		}
	}
}
