...
export function makeExpandable(
  ...
  pill.addEventListener("click", (e: MouseEvent) => {
    ...
    if (contentEl.contains(target) && target !== contentEl) return;
  ...
  document.addEventListener("click", (e: MouseEvent) => {
    if (!pill.classList.contains("pill-expanded")) return;
...
function togglePill(
  pill: HTMLElement, contentEl: HTMLElement,
  ...
  if (pill.classList.contains("pill-expanded")) {
    ...
    for (const other of activePills) {
      ...
      if (otherContent !== null) collapse(other, otherContent);
    }
...
function expand(
  pill: HTMLElement, contentEl: HTMLElement, onExpand?: () => void,
): void {
...
function collapse(
  pill: HTMLElement, contentEl: HTMLElement, onCollapse?: () => void,
): void {
...
export function collapseAll(): void {
  for (const pill of activePills) {
    const contentEl = pill.querySelector(
      ".pill-expand-content",
    ) as HTMLElement | null;
    if (contentEl !== null) collapse(pill, contentEl);
  }
}
