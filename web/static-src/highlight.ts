...
const KEYWORDS: Readonly<Record<string, Set<string>>> = {
  go: new Set(["package", "import", "func", "var", "const", "type", "struct", "interface", "map", "chan", "go", "defer", "return", "if", "else", "for", "range", "switch", "case", "default", "select", "break", "continue", "fallthrough", "nil", "true", "false", "iota", "make", "new", "append", "len", "cap", "delete", "copy", "close", "panic", "recover", "error"]),
  ts: new Set(["import", "export", "from", "const", "let", "var", "function", "class", "interface", "type", "enum", "return", "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "new", "this", "super", "extends", "implements", "async", "await", "yield", "throw", "try", "catch", "finally", "typeof", "instanceof", "in", "of", "void", "null", "undefined", "true", "false", "as", "is", "readonly", "abstract", "static", "private", "protected", "public", "declare", "module", "namespace"]),
  js: new Set(["import", "export", "from", "const", "let", "var", "function", "class", "return", "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "new", "this", "super", "extends", "async", "await", "yield", "throw", "try", "catch", "finally", "typeof", "instanceof", "in", "of", "void", "null", "undefined", "true", "false", "delete"]),
  py: new Set(["import", "from", "def", "class", "return", "if", "elif", "else", "for", "while", "break", "continue", "pass", "raise", "try", "except", "finally", "with", "as", "yield", "lambda", "and", "or", "not", "in", "is", "None", "True", "False", "global", "nonlocal", "del", "assert", "async", "await"]),
  sh: new Set(["if", "then", "else", "elif", "fi", "for", "while", "do", "done", "case", "esac", "in", "function", "return", "exit", "local", "export", "readonly", "unset", "shift", "set", "true", "false", "source"]),
  rs: new Set(["fn", "let", "mut", "const", "struct", "enum", "impl", "trait", "pub", "use", "mod", "crate", "self", "super", "return", "if", "else", "for", "while", "loop", "match", "break", "continue", "as", "in", "ref", "move", "async", "await", "unsafe", "where", "type", "true", "false", "None", "Some", "Ok", "Err"]),
};

// Shared keywords for languages without specific sets
const GENERIC_KW = new Set(["if", "else", "for", "while", "return", "function", "class", "import", "export", "const", "let", "var", "true", "false", "null", "undefined", "new", "this"]);
...
const PUNCT_CODES = new Set<number>([
  123, 125, 40, 41, 91, 93, 59, 44, 46, 58, 33, 38, 124, 60, 62, 61,
  43, 45, 42, 47, 37, 94, 126, 63, 64,
  // { } ( ) [ ] ; , . : ! & | < > = + - * / % ^ ~ ? @
]);
...
function isTokenBoundaryCode(c: number): boolean {
  return isIdentStartCode(c) || isDigitCode(c) || c === 34 || c === 39 || c === 96
    || c === 35 || c === 47 || isPunctCode(c);
}
...
function tokenize(code: string, lang: string): Token[] {
  ...
  while (i < len) {
    ...
    if (cc === 35 /* # */ && (lang === "py" || lang === "sh" || lang === "yaml" || lang === "toml" || lang === "docker")) {
      const end = code.indexOf("\n", i);
      const slice = end === -1 ? code.substring(i) : code.substring(i, end);
      tokens.push({ type: "comment", value: slice });
      i += slice.length;
    ...
    if (cc === 34 || cc === 39 || cc === 96) { // " ' `
      let j = i + 1;
      if (cc === 96) {
        while (j < len && code.charCodeAt(j) !== 96) j++;
        if (j < len) j++;
      } else {
        while (j < len && code.charCodeAt(j) !== cc) {
          if (code.charCodeAt(j) === 92 /* \\ */) j++;
          j++;
        }
        if (j < len) j++;
      }
    ...
    // Numbers
    if (isDigitCode(cc) || (cc === 46 /* . */ && i + 1 < len && isDigitCode(code.charCodeAt(i + 1)))) {
      let j = i;
      if (cc === 48 /* 0 */ && j + 1 < len && /[xXoObB]/.test(code[j + 1]!)) j += 2;
      while (j < len && /[\d.a-fA-F_eE+-]/.test(code[j]!)) j++;
    ...
    if (isIdentStartCode(cc)) {
      let j = i;
      while (j < len && isIdentCharCode(code.charCodeAt(j))) j++;
    ...
    let j = i;
    while (j < len && !isTokenBoundaryCode(code.charCodeAt(j))) j++;
    if (j === i) j = i + 1;
...
export function highlightByLang(code: string, lang: string): string {
  if (lang === "" || lang === "md") return escText(code);
  ...
  for (const t of tokens) {
    if (t.type === "text") html += escText(t.value);
    else html += `<span class="hl-${t.type}">${escText(t.value)}</span>`;
  }
  return html;
}
...
export function normalizeLang(tag: string): string {
  ...
  if (KEYWORDS[s] !== undefined) return s;
