...
func (rp *Runs) LaunchRun(ctx context.Context, source string, inputs map[string]string) (id, name string, err error) {
...
func (rp *Runs) LaunchScheduledRun(ctx context.Context, source, scheduleID string, slotAt time.Time) (id, name string, err error) {
...
func (rp *Runs) launchRun(ctx context.Context, source string, inputs map[string]string, o launchOrigin) (id, name string, err error) {
...
func (rp *Runs) CancelRun(ctx context.Context, workflowID string) error {
	if workflowID == "" {
...
// cancelled state.
func (rp *Runs) cancelRunRPC(ctx context.Context, workflowID string) error {
...
func (rp *Runs) DeleteRun(ctx context.Context, workflowID string) error {
	if workflowID == "" {
		return errors.New("missing workflow id")
	}
...
func (rp *Runs) PauseRun(ctx context.Context, workflowID string) error {
...
func (rp *Runs) ResumeRun(ctx context.Context, workflowID string) error {
...
func (rp *Runs) RetryRun(ctx context.Context, workflowID string) error {
	if workflowID == "" {
		return errors.New("missing workflow id")
	}
...
func (rp *Runs) recipeOfRun(ctx context.Context, workflowID string) string {
...
func (rp *Runs) SetRunStepStatus(ctx context.Context, workflowID, nodeID, status string) error {
	if nodeID == "" {
...
func (rp *Runs) hostedRunControl(ctx context.Context, workflowID, method string) error {
	sb := rp.bridges.get(runChatID(workflowID))
	if sb == nil {
		return errRunNotHosted
	}
...
func (rp *Runs) runControl(ctx context.Context, workflowID, method, logLabel string) error {
...
func (h *Runtime) runDispatch(ctx context.Context, chatID vibekit.ChatID, msg *vibekit.RPCResponse) {
	if msg.ID != nil {
...
func (h *Runtime) runDispatchRequest(ctx context.Context, chatID vibekit.ChatID, msg *vibekit.RPCResponse) {
	switch {
...
// over.
func (h *Runtime) closeFinishedRunBridge(chatID vibekit.ChatID, msg *vibekit.RPCResponse) {
	var p struct {
...
// paused).
func (rp *Runs) resumeRestartPausedRuns(ctx context.Context, chatID vibekit.ChatID) {
...
func (rp *Runs) workflowRunsRaw(ctx context.Context) ([]kasWorkflowRun, error) {
	u := rp.utility()
	cctx, cancel := context.WithTimeout(ctx, sessionListTimeout)
	defer cancel()
...
func (rp *Runs) CancelChatRuns(ctx context.Context, chatID vibekit.ChatID) {
	chat, ok := rp.chats.Get(ctx, chatID)
	if !ok {
		return
	}
