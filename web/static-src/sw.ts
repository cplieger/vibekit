...
sw.addEventListener("notificationclick", ((event: NotificationEvent) => {
  ...
  event.waitUntil(
    sw.clients.matchAll({ type: "window", includeUncontrolled: true }).then((list: readonly Client[]) => {
...
sw.addEventListener("pushsubscriptionchange", ((event: PushSubscriptionChangeEvent) => {
  ...
  event.waitUntil(
    sw.registration.pushManager.subscribe(
      old !== null ? old.options : { userVisibleOnly: true },
