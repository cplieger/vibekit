...
// runPlane owns the workflow-run surface: launch, cancel, retry, the durable
...
// launches.
//
// It exists because that surface was 74 of Hub's 223 methods — a third of the
...
//
// It follows lifecyclePlane and ssePlane, with one difference worth naming: those
...
// named dependencies instead of through a *Hub back-pointer. That was a choice
// with a cost attached: three methods stay on Hub for it (see runDispatch,
...
type runPlane struct {
	...
	// sync.Once whose hooks point back into hub service surfaces (see
...
func (rp *runPlane) hubContext() (context.Context, context.CancelFunc) {
...
//
// One accessor rather than two Hub forwards. A forward would put the hub back in
...
func (h *Hub) Runs() *runPlane { return h.runs }
