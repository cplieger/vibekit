...
func TestBuildACPArgs(t *testing.T) {
	tests := []struct {
		...
		{name: "bare", want: []string{"acp", "--agent-engine", "v2"}},
		{
			name: "agent and model", agent: "kiro_default", model: "claude-opus-4.8",
			want: []string{"acp", "--agent-engine", "v2", "--agent", "kiro_default", "--model", "claude-opus-4.8"},
		},
		{name: "model auto is omitted", model: api.ModelAuto, want: []string{"acp", "--agent-engine", "v2"}},
		{
			name: "valid effort appended", model: "m", effort: "high",
			want: []string{"acp", "--agent-engine", "v2", "--model", "m", "--effort", "high"},
		},
		{
			name: "invalid effort dropped", model: "m", effort: "ultra",
			want: []string{"acp", "--agent-engine", "v2", "--model", "m"},
		},
		{
		...
		},
		{
			...
			want: []string{"acp", "--agent-engine", "v2", "--trust-all-tools", "--effort", "max"},
		},
	}
